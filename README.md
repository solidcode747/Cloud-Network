...
![Diagram](https://raw.githubusercontent.com/solidcode747/Cloud-Network/main/Images/Diagram_Final_Renzo_Tragsiel.png?token=AR7YBNDIRNYCIYD27RFMQETANZAQ6)
...
- Filebeat is a system designed to motitor changes to any and all **files** in the DVWA servers on an on-going basis.
- Metricbeat monitor changes to any and all **system resources** in the DVWA Web servers ongoingly.
- Together, filebeat and Metricbeat and the ELK server increase the security of the servers by carrying out all monitong activities thorugh their respective containers (See diagram above), furter limiting any direct interaction with the DVWA Web Servers themselves, as well as watch system metrics, such as CPU usage; attempted SSH logins; sudo escalation failures; etc.
...
Only the **Jumpbox** machine can accept connections from the Internet. Access to this machine is only allowed from predetermined IP addresses belonging to the system adminstrator team and from computers that have the pgp Key used to create the jumpbox.
...
- Installs needded files
- Configures the server and programs
- laundches the docker container that has the configured Elk Stak (ex: filebeat, metricbeat)
...
- Use a simiar process for the elk-server and when installing metricbeat.
...
- Once inside the ansible directory, run `ls -la` and youll see al the necessary files as in this example: 
...
- Use these commands to run the playbooks:
    ...
    `$ ansible-playbook metricbeat-playbook.yml webservers` (when installing the filebeat container)
...
- Navigate to the specific Docker host you've configured via ssh to check that the installation worked as expected.  Once inside the host VM, You can then run this:
    ...
    This is also shown above along with a sample its the output.
...
- Alterativevly, We can verify that we can load the elk stack server from our web browser at the kibana site by running this:  
